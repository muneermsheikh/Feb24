...
import { candidateParams } from 'src/app/shared/params/hr/candidateParams';
...
@Component({
  ...
  styleUrls: ['./hr-checklist.component.scss']
})
export class HrChecklistComponent implements OnInit{
  ...
  getProfessions() {
    this.masterService.getCategoryList().subscribe(response => {
  ...
  getAgents() {
    this.masterService.getAgents().subscribe(response => {
  ...
  openChecklistModal(candidateid: number) {
      ...
      this.bsModalRef.content.updateChecklist.subscribe((values: IChecklistHRDto) => {
      this.cl = values;
