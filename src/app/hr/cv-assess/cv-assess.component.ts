...
import { HelpModalComponent } from 'src/app/shared/components/modal/help-modal/help-modal.component';
...
import { ICandidateAssessmentWithErrorStringDto } from 'src/app/shared/dtos/hr/candidateAssessmentWithErrorStringDto';
...
import { ChecklistService } from 'src/app/shared/services/hr/checklist.service';
import { CvAssessService } from 'src/app/shared/services/hr/cv-assess.service';
import { BreadcrumbService } from 'xng-breadcrumb';

@Component({
  selector: 'app-cv-assess',
  templateUrl: './cv-assess.component.html',
  styleUrls: ['./cv-assess.component.scss']
})
...
//checklist
  ...
  //emit to child
  orderItemChangedEventSubject: Subject<ICandidateAssessment | null> = new Subject<ICandidateAssessment | null>();
  ...
  chooseSelectedOrderItem() {
    ...
    this.service.getCVAssessmentAndChecklist(this.cvBrief!.id, this.orderItemSelected!.orderItemId)
        .subscribe((response: ICandidateAssessmentAndChecklist | undefined) => 
        {
  ...
  createNewAssessment(){
    ...
    if (!this.requireInternalReview) {    //create assessment header without assessment question items
      ...
      this.assessmentService.getOrderItemAssessmentQs(this.orderItemSelectedId).subscribe(response => {
          items = response;
          if (items.length === 0) {
  ...
  updateAssessment(assess: ICandidateAssessment) {
   ...
   //check data
    ...
    return this.assessmentService.updateCandidateAssessment(assess).subscribe((response: ICandidateAssessment) => {
        if (response) {
  ...
  shortlistForForwarding() {
    ...
    //create new CV Assessment
      return this.service.insertNewCVAssessmentHeader(false,this.orderItemSelectedId, this.cvBrief!.id).subscribe(
  ...
  getChecklistData(){
    if(this.checklist===null || this.checklist?.checklistHRItems.length===0) {
      ...
      this.checklistService.addNewChecklistHR(this.cvBrief!.id, this.orderItemSelectedId).subscribe({
        next: (response: IChecklistHRDto) => {
          this.checklist = response;
          console.log('getchecklistdata checklist', this.checklist);
        },
        error: error => this.toastr.error('failed to retrieve checklist', error)
      })
    } else {
      console.log('byepassed getOrCreatechecklist');
    }

    console.log('checklist after getchecklist:', this.checklist);
