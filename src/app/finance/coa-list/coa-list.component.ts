...
import { DateInputRangeModalComponent } from 'src/app/shared/components/modal/date-input-range-modal/date-input-range-modal.component';
...
export class CoaListComponent implements OnInit {
  ...
  async getCOAs(useCache=false) {
    ...
    return this.service.getCoas(useCache).subscribe({
      next: (response: IPagination<ICOA[]>) => {
