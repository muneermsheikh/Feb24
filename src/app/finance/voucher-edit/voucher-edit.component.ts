...
import { voucherToAddDto } from 'src/app/shared/dtos/finance/voucherToAddDto';
...
export class VoucherEditComponent implements OnInit {
  ...
  //voucher entries
    setExistingVoucherEntries(e: IVoucherEntry[]) {
     e.forEach(x => {
        this.voucherEntries.push(this.fb.group({
...
//voucher attachments
    setExistingVoucherAttachments(v: IVoucherAttachment[]) {
      v.forEach(x => {
        this.voucherAttachments.push(this.fb.group({
          id: x.id,
