...
namespace infra.Services
{
     public class ComposeMessages : IComposeMessages
     {
          ...
          {
               var qry = await (from cvrefs in _context.CVRefs where cvrefs.OrderItemId == orderitemId
                    group cvrefs by cvrefs.RefStatus into grpStatus
                    orderby grpStatus.Key
                    select new OrderItemReviewStatusDto {
                         OrderItemId = orderitemId,
                         CtOfSelected = grpStatus.Count(x => x.RefStatus==(int)EnumCVRefStatus.Selected),
                         CtOfNotReviewed = grpStatus.Count(x => x.RefStatus==(int)EnumCVRefStatus.Referred),
                         CtOfRejected = grpStatus.Count(x => x.RefStatus !=(int)EnumCVRefStatus.Selected 
